...
gerarPdfBtn.addEventListener("click", () => {
  ...
  y += 7;
  pdf.text(`- Departameto: ${doc.responsavel.departamento}`, 25, y);
  y += 15;
  ...
  if(doc.revisoes.length > 0){
    doc.revisoes.forEach((rev, index) => {
        ...
        const maxWidth = 165;
        const comentarioLines = pdf.splitTextToSize(`- Comentário: ${ewv.comentario}`, maxWidth);
...
copyBtn.addEventListener("click", () => {
    ...
    if(textoParaCopiar.trim() === ""){
        alert("Gere um doocumento primeiro para poder copiar!");
        return;
    }
