...
const revisoesContainer = document.getElementById("revisoes_container");
...
function atualizarPalavras(){
    ...
    palavras.forEach(p=>{
        ...
        li.addEventListener("click",()=>{
            palavras= palavras.filter(item !== p);
            atualizarPalavras();
        });
...
function construirDocumento(){
    ...
    const revisoes = revisoesInputs.map(div => ({
        data: FragmentDirective.querySelector(".data_revisao").value,
...
form.addEventListener("submit", e => {
    ...
    const documentMongo = JSON.parse(JSON.stringify(document));
...
gerarPdfBtn.addEventListener("click", () =>{
    ...
    pdf.text(`Data de Envio: ${new Date(doc.data_envio).toLocaleString('pr-BR')}`,120,y);
    y += 15;
